...
def display_result(result):
    ...
    list = result["change"]
    result_tree = ttk.Treeview(root, height=12, columns=(
    ...
    if(string == "INSUFFICIENT_FUNDS"):
        ...
        while(i < 2):
            if(i == 1):
                result_tree.insert(parent='', index='end', iid=i,
                               text="", values=(string), tags="status")
                result_tree.tag_configure('status', font=("Arial", 18))
            else:
                result_tree.insert(parent='', index='end', iid=i,
    ...
    for record in list:
        if(count == 0):
            result_tree.insert(parent='', index='end', text="",
                           values=(" ", " ", " "), tags="status")
            result_tree.insert(parent='', index='end', iid=count, text="", values=(
                string, record[0], record[1]), tags="row")
            result_tree.tag_configure('row', font=("Arial", 18))
        else:
            result_tree.insert(parent='', index='end', iid=count, text="", values=(
                " ", record[0], record[1]), tags="row")
            result_tree.tag_configure('row', font=("Arial", 18))
        count += 1
    result_tree.place(x=(screen_width*44.9)/100, y=(screen_height*9)/100)
